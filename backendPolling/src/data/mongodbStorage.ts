...
export async function updateOrder(
  ...
  try {
    const result = await Order.updateMany(
      { elmaId: identifier }, // filter
      { $set: updateData }            // use $set for safety
    );

    if (result.modifiedCount > 0) {
      const updatedOrder = await Order.findOne({ elmaId: identifier, userId });
      return updatedOrder;
    }

    return null;
  } catch (error) {
    console.error("Error updating many orders:", error);
    return null;
  }
}
...
export async function getOrdersByUserIdWithLimit(
  ...
  try {
    ...
    console.error('Error getting paginated orders by user ID:', error);
...
export async function getOrderById(elmaId: string): Promise<IOrder | null> {
  try {
    return await Order.findOne({ elmaId }).sort({ createdAt: -1 });
  } catch (error) {
    console.error('Error getting orders by user ID:', error);
    return null;
  }
...
export async function createMessage(userId: string, messageData: {
  ...
  try {
    ...
    const message = new Message({
      ...
      isChanged: true,
      createdAt: messageData.__createdAt,
      updatedAt: messageData.__updatedAt
    });
...
export async function addCommentToMessage(
  ...
  try {
    const messages = await Message.find({ __id: messageId });

    if (!messages.length) {
      console.warn(`⚠ No messages found with __id=${messageId}`);
      return null;
    }
    ...
    await Promise.all(
      messages.map(async (message) => {
        const alreadyExists = message.comments?.some(
          (el) => el.__id === commentData.__id
        );

        if (alreadyExists) {
          console.log(
            `ℹ Comment ${commentData.__id} already exists in message ${message._id}`
          );
          return; // skip updating this one
        }

        message.comments.push({
          __id: commentData.__id,
          authorId: commentData.authorId,
          body: commentData.body,
          createdAt: commentData.__createdAt ?? new Date().toISOString(),
        });

        message.unreadCommentsCount = (message.unreadCommentsCount || 0) + 1;
        message.isChanged = true;

        if (commentData.__createdAt) {
          message.updatedAt = commentData.__createdAt
        }
    ...
    console.log(`💬 Added comment ${commentData.__id} to ${updatedMessages.length} messages`);
    return updatedMessages?.[0];
  } catch (error) {
    console.error("❌ Error adding comment to messages:", error);
    return null;
  }
}
...
export async function getMessagesByUserId(userId: string): Promise<IMessage[]> {
  try {
    ...
    // Убираем дубликаты по __id
    const uniqueMessages = Array.from(
      new Map(messages.map(msg => [msg.__id, msg])).values()
    );
...
export async function getMessagesByOrderId(targetId: string): Promise<IMessage[]> {
  try {
    return await Message.find({targetId}).sort({ createdAt: -1 });
    } catch (error) {
...
// * Update orderIds in a specific WebSocket session
export async function updateWebsocketSessionOrders(id: string, orderIds: any) {
  try {
    // Приводим orderIds к нормальной форме (массив строк)
    let normalizedOrderIds: string[];

    if (typeof orderIds === "string") {
      // если пришло как строка "[ '...', '...' ]"
      try {
        const parsed = JSON.parse(orderIds);
        normalizedOrderIds = Array.isArray(parsed[0]) ? parsed[0] : parsed;
      } catch {
        normalizedOrderIds = [orderIds];
      }
    } else if (Array.isArray(orderIds)) {
      // если случайно массив массивов
      normalizedOrderIds = Array.isArray(orderIds[0]) ? orderIds[0] : orderIds;
    } else {
      normalizedOrderIds = [];
    }
    ...
    const session = await WebsocketSession.findOneAndUpdate(
      { id },
      { $set: { orderIds: normalizedOrderIds } },
      { new: true }
    );

    console.log(`✅ Updated orderIds for session id=${id}`, normalizedOrderIds);
    return session;
  } catch (err) {
    console.error("❌ Error updating websocket session orderIds:", err);
  }
}
