...
export async function getMessages(userId: string, order: ELMATicket, token: string, cookie: string, prevMessageCount: number): Promise<{isNewMessage: boolean, messages?: any[], authors?: Record<string, string> | undefined} | undefined> {
    ...
    messages.forEach((message: any) => {
      messageCount += 1
      if (message.comments) {
        messageCount += message.comments.length;
      }
    })
    ...
    return {isNewMessage: true, messages, authors};
...
export async function getPassports(userId: string, mergedOrders: any[]): Promise<Record<string, [string | undefined, string | undefined]>> {
    ...
    await Promise.all(
        Array.from(AllPassports).map(async (passport) => {
            ...
            await createPassport(userId, {passportId: data.item.__id, passportData: data.item.dannye_pasporta ?? '', name: data.item.familiya_imya_po_pasportu });
            } catch (error) {
...
export async function getAnotherUsers(savedClientId: string) {
  const responseUser = await axios.post(
    'https://portal.dev.lead.aero/pub/v1/app/_system_catalogs/_user_profiles/list',
    {
      "active": true,
