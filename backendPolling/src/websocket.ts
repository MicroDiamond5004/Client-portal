  ...
  wss.on("connection", (ws: WebSocket) => {
    ...
    ws.on("message", async (data) => {
      try {
        ...
        if ((msg.type === "init" || msg.type === "register") && msg.userId && msg.email) {
        ...
        else if (msg.type === "updateOrders" && msg.email && Array.isArray(msg.orderIds)) {
          const { email, orderIds, id } = msg;

          if (!id) {
            console.log("⚠ Missing socket ID in updateOrders");
            return;
          }

          const connections = socketConnections.get(email);
          if (!connections) {
            console.log(`⚠ No active connections for ${email}`);
            return;
          }

          const connection = [...connections].find((conn) => conn.id === id);
          if (!connection) {
            console.log(`⚠ No active connection with id=${id} for ${email}`);
            return;
          }

          // Update only this specific session in MongoDB
          await updateWebsocketSessionOrders(id, orderIds);

          // Confirm only to this client
          ws.send(
            JSON.stringify({
              type: "ordersUpdated",
              orderIds,
          ...
          console.log(`✅ Updated orderIds for ${email}, id=${id}`);
        }
